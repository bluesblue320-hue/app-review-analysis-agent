...
| `expected_tools` / `expected_tools_all` | 必须调用的工具 |
| `expected_tools_any` | 多个合理工具中至少调用一个 |
...
| Tool Exact Match | 实际工具集合与预期集合完全一致的案例占比（仅参考，不判失败） |
| Tool Precision | 正确调用工具数 / 实际调用工具数（按案例平均） |
| Tool Recall | 正确调用工具数 / 预期工具数（按案例平均） |
...
| Illegal Tool Block Rate | 对抗案例中非法工具均被拒绝的比例，目标 100% |
...
4. 运行 `python -m evaluation.evaluate_agent --case-id <id>` 验证；
5. 在 `tests/test_agent_evaluation.py` 中补充覆盖（如需）。
